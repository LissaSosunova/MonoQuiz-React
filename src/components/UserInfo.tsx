...
function UserInfo({ isOpened, isMobile, onToggle, onClose }: UserProps) {
    ...
    return (
        <div className={`proofile-info-block ${!isMobile ? 'd-flex-center-center' : ''}`}>
            <div
                className={`btn-profile ${isMobile ? 'hide' : ''}`}
                onClick={onToggle}
            />
            {isMobile && (
                <div
                ...
                >
                    Name: {request.session.customer_name} Email: {request.session.customer_email}
                </div>
            )}
            {!isMobile && (
                <div
